...
```json
{
  ...
  "content": {
    "left_panel": {
      ...
      "cards_layout": "5 cards in a straight horizontal row, side by side, no arc, no curve, no rotation, no perspective, position 3 = center largest",
      "bottom_button": "立即抽奖",
...
- Top: brand logo "{{content.left_panel.logo}}" in the SAME style/position/size as LEFT panel
- Below logo: subtitle "{{content.left_panel.subtitle}}" in the SAME style/position as LEFT panel
- Center-top: focus character ({{content.left_panel.focus_character}}) in the SAME position as LEFT panel
- Background: {{content.left_panel.background}}, in the EXACT SAME style/colors/composition as LEFT panel
...
1. top_button: {{color_palette.accent}} ? round button (36x36, SAME size as LEFT panel) — labeled "{{content.left_panel.top_button}}"
2. coupon_position_1: {{coupons.C1.name}} {{coupons.C1.discount}} (SAME height as LEFT panel position 1)
3. coupon_position_2: {{coupons.C3.name}} {{coupons.C3.discount}} (SAME height as LEFT panel position 2)
4. coupon_position_3: {{coupons.C5.name}} {{coupons.C5.discount}} (largest, gold border, NO glow, NO sparkles, SAME height as LEFT panel position 3)
5. coupon_position_4: {{coupons.C4.name}} {{coupons.C4.discount}} (NO glow, NO sparkles, SAME height as LEFT panel position 4)
6. coupon_position_5: {{coupons.C2.name}} {{coupons.C2.discount}} (SAME height as LEFT panel position 5)
7. cta_button: {{color_palette.accent}} rounded "{{content.left_panel.bottom_button}}" button, {{color_palette.base}} text (SAME size as LEFT panel cta_button)
...
- All cards SAME border style ({{color_palette.base}} bg + {{color_palette.primary}} text + {{color_palette.accent}} border), EXCEPT Position 3 (center) which uses {{color_palette.highlight}} gold border for visual distinction. NO highlight glow, NO star sparkles, NO halo, NO aura on any card. NO highlight_glow element anywhere.
...
- {{visual_style}}, soft gradient, high quality, commercial-grade product design.
```
...
| Rule 6 | **无任何 highlight glow / 星星 / 光晕**。所有卡不带光晕。卡 3 唯一视觉差异：金边 + 最大 1.18x |
| Rule 7 | 无禁用 UI |
| Rule 8 | 无真实资产 |
| Rule 9 | 中区只有背景+角色 |
| Rule 10 | 右区 7 元素（1 圆按钮 + 5 卡 + 1 CTA）。**无 highlight_glow 元素，无 #FF00FF 切图参考线** |
| Rule 11 | **右区 5 张卡与左区 5 张卡位置 x/y/w/h 完全一致**（pixel-perfect mirror，同一高度） |
| Rule 12 | **右区按钮与左区按钮大小/位置一致**（top_button 36x36 + cta_button 大圆角） |
| Rule 11 | 中文清晰 |
...
- **v2.9 (2026-06-15)**: 修大g 反馈 2 个问题（便利店 demo 阶段）
  ...
  2. **右区卡高度 = 左区卡高度**（pixel-perfect mirror）：RIGHT panel 卡 P1-P5 的 HEIGHT 显式强调 `SAME height as LEFT panel position N`
  3. **右区按钮大小 = 左区按钮**：`top_button 36x36` + `cta_button` 大圆角在 RIGHT panel 显式标记 `SAME size as LEFT panel`
  4. **QA Gate 加 Rule 12**：右区按钮与左区按钮大小/位置一致

- **v2.8 (2026-06-15)**: 修大g 反馈 4 个问题（便利店 demo v4 阶段）
  1. **卡 3、4 不带高光光晕**：`(with highlight glow + star sparkles)` → `NO highlight glow, NO star sparkles`
  2. **右区删 highlight_glow 元素**：element_count 8 → 7
  3. **删 #FF00FF 紫色切图参考线**：`each with 2px #FF00FF border` → `CRITICAL — NO magenta cut guides`
  4. **右区卡位置 = 左区卡位置**：`EXACTLY match the size, position` → `EXACT SAME x, y, width, height (pixel-perfect mirror)`

- **v2.7 (2026-06-15)**: 修大g 反馈 milktea 残留（便利店 demo 阶段）
  1. **MIDDLE PANEL 全部用占位符**：`"传招冷饮" "夏日冰爽特惠" "Q版奶茶杯戴草帽" "warm wood counter" "sunbeams" "mint leaves" "ice cubes"` → `{{content.left_panel.logo}}` `{{content.left_panel.subtitle}}` `{{content.left_panel.focus_character}}` `{{content.left_panel.background}}`
  2. **Padding 字段去重**：`"~13px greenscreen"` → `"~13px"`（避免渲染出 "greenscreen greenscreen"）
  3. **删 "refreshing summer" 残留**：→ `high quality, commercial-grade product design`
  4. **加 pixel-aligned 约束**：LEFT panel 和 MIDDLE panel 元素像素对齐
  5. **加 BaseCard 修饰**：明确 S1/S2/S3 是从同一个 BaseCard 母版缩放
